...
- **Protocol change mid-fast**: the user is asked. Changing plan while a fast is running presents an alert naming both hours — apply the new goal to the fast in progress as well, or leave that fast on the goal it started with — and the new protocol applies from the next fast either way. Cancel changes nothing. The snapshot rule is otherwise unchanged: `goalHours`/`protocolID` never move on their own, and no completed fast is ever re-judged. Choosing a goal the fast has already passed is allowed; it simply reads as reached.
