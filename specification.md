...
- **The ring behind the sheet previews the time being offered.** While the start sheet is open the ring draws the fast as if it had begun at the chosen time; while the end sheet is open it stops at the chosen end, so the elapsed count, the zone and the fill are the ones the confirm button would record. The preview is silent — no zone flash, no goal celebration, no haptic — because a time still being nudged is not a crossing that happened.
...
- **Starting a fast on the watch silences the phone's reminder immediately.** The start reminder is armed on the phone and only the phone can cancel it, so the watch tells it over WatchConnectivity — the one fact that doesn't travel through iCloud, because a suspended phone runs no code and CloudKit's push is best-effort. Without it, a fast started at 19:45 on the watch still gets a "time to start your fast?" at 20:00. What crosses is scheduling state, never data: the fast itself still arrives as a CloudKit import, and the phone re-derives everything from its own store the moment one lands.
...
The foot of the Stats screen, under the week strip and the History row. **Two panels** over the last 30 days: **weight over fasting hours** on a date axis, and **stopped eating & sleep** — one dot a night, when eating stopped against how long the sleep after it was. Each panel is one pairing the user can act on — a thing they chose (how long they fasted, when they stopped eating) and the thing worth reading it against (weight, sleep) — because reading them against each other is the entire point.

**Two panels, and no more.** There is no fitted line, no correlation, no computed headline telling the user what their data means: that reading was tried and cut for being more machinery than anyone wants on a fasting app's stats page. These panels show what happened; the user does the interpreting. A scatter of what happened is on the right side of that line — a slope the eye finds in thirty dots is the user's reading, not the app's.

The two panels reach their pairing differently, and deliberately.

**Weight and fasting hours genuinely have no common unit**, so that panel is dual-axis, under a contract that keeps the two scales honest: the **bars are hours fasted**, grounded at zero and read against the **right axis**; the **weight line is read against the left**. The two are never expressed in each other's units, so a crossing is meaningless by construction. Mechanically the chart is drawn in the bars' scale and weight is projected into it with every label converted back; only the hours axis draws grid lines, so a bar top can never be misread in kilograms. Nothing claims one series explains the other.

**The sleep panel has no second scale to keep honest either**, because its two facts are its two axes. It has been drawn two other ways and both failed the one question it exists for — does stopping eating earlier go with sleeping longer. Dual-axis first: the stop dots had no scale of their own, so their height was borrowed from the sleep-hours scale and whatever shape the eye found in them was an artifact of the projection. Then a timeline on one clock axis: a dot where eating stopped and a band from falling asleep to waking, thirty dated columns of them. Honest, and unreadable for the purpose — hours asleep only existed as the length of a band, which the eye can't compare across thirty columns, and a month of real data couldn't be told either way.

- **The sleep panel is a scatter: one dot a night.** Across, the clock time eating stopped; up, the hours asleep that followed. An earlier-stop-longer-sleep month is a cloud that slopes down to the right; a month where it made no difference is a cloud with no slope. Either is visible at a glance, which is the whole reason for the form.
  - **No fitted line, no coefficient, no headline.** The dots are the data; the slope is the user's to see or not. This is what the "no correlations" rule allows and the cut Trends screen didn't respect.
  - **One summary, and it is a pair of averages, not a fit**: the mean hours asleep on the nights that stopped by the planned time, and on the nights that stopped after it — drawn as a level running each side of the dashed rule, and set out under the chart as **two stat tiles** in the bento's face — "STOPPED BY 20.00 / 7h 20m / average over 14 nights" beside "AFTER 20.00 / 6h 35m / average over 12 nights". Tiles rather than a caption: the numbers are the panel's answer, and a sentence in the caption face was the least visible thing on the screen. Two facts about nights that happened, read against a line the user drew; the reader supplies the "so", and the caption never says "because". Without an anchor the split falls on the user's median stop and says so.
    - Two groups rather than clock-hour bins: thirty nights split six ways is groups of two or three, and an average of two nights is noise dressed as a fact.
    - **Nothing at all unless both sides have at least three nights.** With less, the scatter stands alone.
  - The stop is plotted as **signed hours from that day's midnight**, so a 20:00 stop the evening before is −4, not 20. A wrapped 0–24 axis would put 23:50 and 00:10 at opposite ends, which is exactly the pair a late eater needs to see as neighbours. Later is to the right.
  - Both axes are labelled at whole hours — the stop axis at clock times, the sleep axis in hours — and the stop axis is never allowed to span more than a day, since a wider one would print the same time twice.
  - **The sleep axis is not zero-based.** Sleep lives between five and nine hours; a zero base would spend half the frame on hours nobody sleeps and squash the real nights into the top of it. It frames the nights on whole hours instead.
  - The dot's height is the day's **total asleep** — naps and all, overlaps counted once — because "how much did I sleep" is the question, and a nap after a short night is part of the answer. The night's *main sleep* (first dropping off to finally getting up, with breaks under three hours counted as the same sleep) is still computed and still what `SleepAggregator` is tested on; it is no longer drawn.
  - **Only nights with both a stop and a sleep are drawn.** A day with one and not the other has nothing to plot against. A missing sleep series says so under the title rather than leaving a panel that merely looks emptier.
  - A stop far outside the drawn scale is **clipped to the plot area** rather than drawn beside the chart. It is still in the accessibility tree.
  - **A stop far from the user's usual evening doesn't set the scale.** A 36-hour fast started on a Monday morning lands sixteen hours from the credited midnight; letting it size the axis would pile every real night into one column. It is still drawn and still read out — it just doesn't pull the frame apart.
  - The eating stop comes from our own data — the start of the fast credited to that day — not from Health.
  - The user's **start-time anchor** (§4.1) is a dashed vertical rule — the time they meant to stop, so a dot to its right reads as late. It's context, not data: if the anchor is so far from the actual times that including it would flatten the dots, it's dropped rather than allowed to squash the scale.
  - VoiceOver reads each dot as its date, the time eating stopped and the hours asleep.
...
- **Every day is the day a fast ended** — the goal-day rule (§2), applied to both panels. Sleep is credited to the day the user woke, and an evening's last meal to the morning it ends at, so one night's eating stop, fast length and sleep are one day's facts — a column in the top panel, a dot in the bottom one. Where several fasts ended on a day, the longest wins, so the bar and the dot always describe the same fast.
