...
Launch with `-seedDemo` (DEBUG only) to populate a streak plus an active fast for screenshots; `-seedEating` seeds the between-fasts state instead. `-tab stats` / `-tab settings` (DEBUG only) opens straight onto another tab — the custom tab bar can't be tapped from `simctl`, so this is how the other screens get screenshotted. `-screen trends` pushes on through to the Trends screen, backed by `FixtureHealthProvider` rather than HealthKit (a simulator's Health store is empty and its permission sheet can't be tapped); add `-noHealthData` to see that screen's empty states, and `-seedHistory` for three months of fasts (the Patterns cards need them before they'll say anything). All three live in `DebugLaunch` in `RootView.swift`.
...
- `Shared/Health/` + `Fastino/Health/` — the Apple Health read path (§4.8), split across the folder boundary on purpose. `Shared/` holds the pure half: `HealthSample` (the value types), `SleepAggregator`, `WeightSeries`, and the `HealthProvider` protocol with its `FixtureHealthProvider`. `Fastino/Health/HealthKitProvider.swift` is **the only file in the app that imports HealthKit**, and it's iOS-only by placement.
  ...
  - **`SleepNight` carries both *how much* and *when*** — the total asleep, plus the start and end of the night's **longest** stretch. Longest rather than first-to-last: a nap would otherwise stretch the night from last evening to this teatime.
  - `FastingEngine.eatingStops` is the series that isn't from Health — the *start* of the fast credited to each day, i.e. when eating stopped. It shares a panel and a clock axis with sleep, which is drawn as *when* (a bar from asleep to awake) rather than *how much* — that's what lets two series overlay without a second scale. Both are in signed hours from that day's midnight (evening = negative) rather than a 0–24 clock, because a wrapped axis puts 23:50 and 00:10 at opposite ends and would cut every night's bar in half. It picks the longest fast per day, like `lastDays`, so the dot and the bar are the same fast.
  - **`Engine/Correlation.swift` + `Health/HealthCorrelation.swift` are the Patterns cards** (§4.8). `Correlation` is domain-free least-squares + Pearson r; `HealthCorrelation` builds the two pairings *and the sentences*, so the wording is unit-tested — a scatter that looks plausible under a headline saying the opposite is the failure that matters here.
    - **The fasting/weight pairing is weekly over 90 days, not daily over 30.** Weight swings ±1 kg on water alone and responds over weeks; a daily plot would be noise dressed as a finding. That's why `HealthProvider.series` takes a separate `weightDays`.
    - **Four refusals**: under 8 pairs, under |r| 0.2, under a 10-minute / 200-gram group difference — and the fit line is drawn *only* when a headline survives all of them. Don't add a code path that draws the line independently.
...
- **Swift Charts is used on exactly one screen — Trends.** Thirty aligned bars plus a line with a real date axis is what it's for. Everywhere else the charts are hand-drawn, and the seven-day strip stays that way because it's a designed object rather than a plot. When you do reach for Charts, every colour, font and gridline still comes from `Theme` — no default Charts palette, no system blue.
