...
Launch with `-seedDemo` (DEBUG only) to populate a streak plus an active fast for screenshots; `-seedEating` seeds the between-fasts state instead. `-tab stats` / `-tab settings` (DEBUG only) opens straight onto another tab — the custom tab bar can't be tapped from `simctl`, so this is how the other screens get screenshotted. `-fixtureHealth` backs the Stats screen's Health panels with `FixtureHealthProvider` rather than HealthKit (a simulator's Health store is empty and its permission sheet can't be tapped); add `-noHealthData` to see their empty states, and `-seedHistory` for three months of fasts. They live in `DebugLaunch` in `RootView.swift`. Note the panels sit *below* the bento and the week strip, so `simctl` — which can't scroll — only sees the top of them; to look at the whole section, temporarily hoist `HealthPanels` to the top of `StatsView.content` and put it back afterwards.
