...
- `Fastino Watch App Watch App/` — the watch app (§4.7). `WatchRootView` is a three-page `TabView(.page)` — `[Progress] ← [Timer] → [Settings]` — opening on the timer, so the fast is always one swipe away and never behind a back button.
  ...
  - `WatchSettingsView` / `WatchProgressView` / `WatchAdjustEndView` — **these exist because the watch app is installable without the phone.** A standalone watch has no other way to pick a protocol or fix a forgotten end time. Keep them a subset: no appearance (there's no light appearance on watchOS), no start-time editing, no export. Anything phone-sized belongs on the phone.
