...
- `Intents/FastIntents.swift` — `Start`/`End`/`ToggleFastIntent` + `AppShortcutsProvider`. `ToggleFastIntent` requests confirmation (it's the Back Tap target).
- `Notifications/NotificationManager.swift` — goal-reached notification (scheduled at start+goal, cancelled on end/edit) and the start reminder (on by default, suppressed while fasting). Permission is requested lazily, never on launch.
  - The start reminder fires at the **earlier** of the eating window's close and the user's chosen daily time, one nudge per day. Dates come from the pure `FastingEngine.startReminderDates`; `NotificationManager` just schedules what it's handed. It's a rolling week of one-shots, not one repeating trigger, so re-arming on launch/foreground/write is what keeps it alive — see `FastStore.reconcileNotifications`.
...
- **The eating window is the exception to that** — it's forward-looking (when is the *next* fast due), so it uses the *current* protocol's `eatingHours`, not the ended fast's. It goes stale after 24h so the timer falls back to "Ready" rather than showing a wildly overrun window.
